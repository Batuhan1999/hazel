...
export interface TestCallViewProps { // Renamed interface for clarity
  isOpen: boolean;
  onClose: () => void;
  roomId: string;
...
export function TestCallView(props: TestCallViewProps) {
  ...
  return (
    <Show when={props.isOpen}>
      <Dialog open={props.isOpen} onOpenChange={(detail: { open: boolean }) => { if (!detail.open) props.onClose(); }} modal>
        ...
        <Dialog.Content class="flex h-[80vh] w-[90vw] max-w-4xl flex-col p-0">
          <Dialog.Title class="border-b p-4 text-lg font-semibold">Video Call</Dialog.Title>
          ...
          <div class="flex items-center justify-center gap-2 border-t p-4 sm:gap-4">
            ...
            <Button intent="destructive" onClick={props.onClose}>End Call</Button>
          </div>
          <Dialog.CloseTrigger class="absolute right-2 sm:right-4 top-2 sm:top-4">
            {/* <XIcon /> */} Close
          </Dialog.CloseTrigger>
        </Dialog.Content>
      </Dialog>
